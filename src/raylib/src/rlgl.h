...
// Custom Instancing functions - Chris
RLAPI void rlBindVAO(void);
RLAPI void BeginModeInstanced(int intanceCount);
RLAPI void EndModeInstanced();
...
//----------------------------------------------------------------------------------
// Module Functions Definition - Instancing feature - Chris
//----------------------------------------------------------------------------------
void rlBindVAO(void)
{
    glBindVertexArray(RLGL.currentBatch->vertexBuffer->vaoId);
}

void BeginModeInstanced(int instanceCount)
{
...
void rlDrawMesh(Mesh mesh, Material material, Matrix transform)
{
    ...
    for (int eye = 0; eye < eyesCount; eye++)
    {
        ...
        if (RLGL.State.drawInstanced)
        {
            ...
            if (mesh.indices != NULL)
            {
                glDrawElementsInstanced(GL_TRIANGLES, mesh.triangleCount*3, GL_UNSIGNED_SHORT, 0, RLGL.State.instanceCount); // Indexed vertices draw
            }
            else
            {
                glDrawArraysInstanced(GL_TRIANGLES, 0, mesh.vertexCount, RLGL.State.instanceCount);
            }
        }
        else
        {
        ...
        if (batch->vertexBuffer[batch->currentBuffer].vCounter > 0)
        {
            ...
            for (int i = 0; i < batch->drawsCounter; i++)
            {
                ...
                if ((batch->draws[i].mode == RL_LINES) || (batch->draws[i].mode == RL_TRIANGLES))
                {
                     if (RLGL.State.drawInstanced)
                    {
                        glDrawArraysInstanced(batch->draws[i].mode, vertexOffset, batch->draws[i].vertexCount, RLGL.State.instanceCount);
                    }
                    else
                    {
